...
let canonicalTechnicalBundles=5;
if(!base){
...
const failed=checks.filter(c=>!c.searchable||c.stockDetail!=='AVAILABLE'||c.history!=='AVAILABLE'||c.quantEvidence!=='AVAILABLE'||c.quantScore!=='UNAVAILABLE'||c.technicalEvidence!=='AVAILABLE'||c.setupStateContract!=='VISIBLE'||c.setupStateActive);
const report={measuredAt:new Date().toISOString(),target:base||'LOCAL_CHECKOUT',sourceArtifactGeneratedAt:summary.generatedAt,
 ...
 ELLIOTT_VISIBLE_UNIVERSE:canonicalTechnicalBundles,
 ...
 measurementBasis:'Canonical product capability summary plus live consumer-service probes; full Technical/Elliott count is the canonical real bundle inventory.'};
