  ...
  switch (operation) {
    ...
    case "BULK_UPLOAD":
      return {
        ...
        objectHeads: toFetch,
        objectReads: 2 * toFetch,      /* identity check and appendSeries each read */
    ...
    case "RECOVERY":
      return {
        kind: operation,
        objectWrites: 0, objectHeads: 0, objectReads: totalSymbols,
        listPages: 0, indexWrites: 0, bytesDelta: 0
      };
...
async function main() {
  ...
  const plan = (() => {
    ...
    const toFetch = ["DAILY_UPDATE", "BULK_UPLOAD", "RECOVERY"].includes(OPERATION) ? tickers.length : p.counts.toFetch;
  ...
  if (!executionAllowed) {
    console.log("\n  PLANNING_ONLY: measured monthly accounting required; no execution budget issued.");
