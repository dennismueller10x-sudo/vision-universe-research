 ...
 */
function loadBudget() {
  if (DRY_RUN && LOCAL_ROOT) {
    /* Ein lokaler Trockenlauf schreibt nichts. Er darf trotzdem lesen, um zu
...
function universeMembers() {
  ...
  if (!Array.isArray(list) || !list.length) throw new Error("HISTORY_UNIVERSE_IDENTITY_INVALID");
  const ids = new Set(), tickers = new Set();
  for (const s of list) {
    if (!s || typeof s.securityId !== "string" || !s.securityId || /[\\/]/.test(s.securityId) ||
        typeof s.ticker !== "string" || !s.ticker || ids.has(s.securityId) || tickers.has(s.ticker)) {
      throw new Error("HISTORY_UNIVERSE_IDENTITY_INVALID");
    }
...
function assertSeries(member, series) {
  if (!series || series.ticker !== member.ticker || series.securityId !== member.securityId ||
      series.provider !== PROVIDER || !Array.isArray(series.bars)) {
    throw new Error("HISTORY_IDENTITY_MISMATCH");
  }
  let previous = null;
  for (const bar of series.bars) {
    if (!bar || bar.securityId !== member.securityId) throw new Error("HISTORY_IDENTITY_MISMATCH");
    const date = typeof bar.date === "string" && /^\d{4}-\d{2}-\d{2}$/.test(bar.date) ? Date.parse(bar.date + "T00:00:00Z") : NaN;
    if (!Number.isFinite(date) || new Date(date).toISOString().slice(0,10) !== bar.date ||
        (previous !== null && bar.date <= previous)) throw new Error("HISTORY_INVALID_DATES");
    previous = bar.date;
  }
}
...
async function main() {
  ...
  if (PULL) {
    ...
    const results = await pool(members, CONCURRENCY, async (m) => {
      ...
      assertSeries(m, series);
      const file = cacheFile(m.securityId);
      const existing = existsSync(file) ? JSON.parse(readFileSync(file, "utf8")) : null;
      if (existing) {
        assertSeries(m, existing);
        if ((existing.adjustmentStatus ?? null) !== (series.adjustmentStatus ?? null)) {
          throw new Error("HISTORY_RECONCILIATION_REQUIRED");
        }
      }
      const byDate = new Map(series.bars.map(bar => [bar.date, bar]));
      for (const bar of existing?.bars || []) {
        if (byDate.has(bar.date) && !isDeepStrictEqual(byDate.get(bar.date), bar)) {
          throw new Error("HISTORY_RECONCILIATION_REQUIRED");
        }
      ...
      if (!DRY_RUN) {
        ...
        // Restore time is not market freshness. Keep local evidence, never
        // fabricate checkpoints, corporate-action checks or provider finality.
        const payload = { ...(existing || {}),
          ticker: series.ticker, securityId: series.securityId, provider: series.provider,
  ...
  if (PUSH) {
    ...
    const results = await pool(members, CONCURRENCY, async (m) => {
      ...
      assertSeries(m, payload);
      if (!payload.bars.length) return { skipped: true, ticker: m.ticker };
