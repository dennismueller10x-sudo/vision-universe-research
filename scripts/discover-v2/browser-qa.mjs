...
for(const width of (engine==='webkit'?[390]:[320,390,1440]))for(const colorScheme of (engine==='webkit'?['dark']:['light','dark'])){
 ...
 await check(key+' responsive navigation remains reachable',async()=>{
  ...
  if(width<500){assert.equal(state.position,'fixed');assert(Math.abs(state.box.bottom-state.height)<=1,'Mobile navigation detached from viewport edge');assert(state.labels.every(i=>i.width>=44&&i.height>=44),'Mobile targets smaller than 44px');assert(state.paddingBottom>=0);}
