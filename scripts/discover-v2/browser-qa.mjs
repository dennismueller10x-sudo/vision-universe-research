...
async function premiumMobileAudit(page,key){
 ...
 const semanticTokens=await page.evaluate(()=>{const style=getComputedStyle(document.body),probe=value=>{const n=document.createElement('i');n.style.color=value;document.body.append(n);const color=getComputedStyle(n).color;n.remove();return color;};return {positive:probe('var(--v2-green)'),negative:probe('var(--v2-red)'),neutral:probe('var(--v2-muted)')};});
