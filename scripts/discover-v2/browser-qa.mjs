...
for(const width of (engine==='webkit'?[390]:[320,390,1440]))for(const colorScheme of (engine==='webkit'?['dark']:['light','dark'])){
 ...
 if(engine==='chromium'&&width===390&&colorScheme==='light'){
  ...
  await check(key+' worlds are distinct full-screen discovery entrances',async()=>{await page.goto(base+'/discover-v2/#/welten',{waitUntil:'networkidle'});const doors=page.locator('.v2-world-directory .v2-world-door');assert(await doors.count()>=8,'World directory needs multiple data-backed entrances');const styles=await doors.evaluateAll(nodes=>nodes.slice(0,4).map(n=>getComputedStyle(n).backgroundImage||getComputedStyle(n).backgroundColor));assert(new Set(styles).size>=3,'World entrances lack visual differentiation');const first=await doors.first().boundingBox();assert(first&&first.height>=page.viewportSize().height*.78,'Mobile world module does not fill the screen: '+(first&&first.height));await screenshot(page,key+'-worlds');await page.goto(base+'/discover-v2/',{waitUntil:'networkidle'});await page.locator('.v2-stock-hero').first().waitFor();});
 ...
 await check(key+' valuation building blocks use canonical metrics',async()=>{const block=page.locator('.dv2-valuation-components');assert(await block.count()===1,'Valuation building-block module missing');const labels=await block.locator('.dv2-valuation-card span').allTextContents();assert(labels.includes('KGV')&&labels.includes('KUV')&&labels.includes('Free-Cashflow-Rendite'),'Expected AAPL valuation components missing: '+labels.join(', '));});
 ...
 if(engine==='chromium'&&width===390&&colorScheme==='light')await check(key+' CrowdStrike business copy and zero-line chart scale',async()=>{
  await page.goto(base+'/discover-v2/#/s/US_REAL/CRWD',{waitUntil:'networkidle'});await page.locator('.dv2-stock-business').waitFor();
