...
for(const width of (engine==='webkit'?[390]:[320,390,1440]))for(const colorScheme of (engine==='webkit'?['dark']:['light','dark'])){
 ...
 const entryStarted=Date.now();await page.goto(base+'/discover-v2/',{waitUntil:'domcontentloaded'});await page.locator('.v2-hero-track .v2-stock').first().waitFor({state:'visible'});
 ...
 await check(key+' incremental view resource budget',async()=>{const resources=performance.at(-1).resources.filter(r=>new URL(r.url).pathname.startsWith('/discover-v2/'));assert(resources.reduce((n,r)=>n+r.bytes,0)<=180000,'New view scripts/styles exceed 180 KB decoded');assert(resources.length<=12,'New view adds more than 12 requests');});
