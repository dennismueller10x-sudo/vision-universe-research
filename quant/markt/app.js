...
(function (global) {
  ...
  function seriesSection(payload) {
    ...
    var adjustmentNote = payload.adjustmentStatus === "adjusted"
      ? "Total-Return-bereinigt."
      : payload.adjustmentStatus === "splitAdjusted"
        ? "Splitbereinigt, aber nicht dividendenbereinigt. Die ausgewiesenen Veraenderungen " +
    ...
    return el("section", { class: "q-section" }, [
      el("div", { class: "q-section-head" }, [
        el("div", {}, [
          ...
          el("p", { text: payload.exchange + " · " + payload.currency + " · " +
                          bars.length + " Handelstage · " + adjustmentNote })
        ]),
      ...
      el("div", { class: "q-chart-wrap" }, [
        C.lineChart({
          title: "Kursverlauf " + payload.ticker,
          description: "Schlusskurse in " + payload.currency + " von " +
                       dates[0] + " bis " + dates[dates.length - 1] + ".",
          dates: dates,
          series: [{ values: closes, className: "line-primary" }],
  ...
  function render() {
    ...
    Q.loadMarketStatus().then(function (status) {
      Q.clear(root);
