...
test('Radar projects materialized signal transitions without activating Quant V2 or Market Regime',async()=>{const radar=await materializedApi.getRadarIntelligence({lookback:20,limit:12});assert.equal(radar.state,'AVAILABLE');assert.equal(radar.scope,'CANONICAL_PRODUCT_UNIVERSE');assert.equal(radar.coverage.requested,6875);assert.equal(radar.coverage.available,5888);assert.equal(radar.modules.length,4);assert.ok(radar.modules.every(module=>module.items.length<=12));assert.ok(radar.modules.flatMap(module=>module.items).every(event=>['ENTERED','EXITED'].includes(event.transition)&&event.query));assert.equal(radar.quantScore.state,'UNAVAILABLE');assert.equal(radar.quantScore.reason,'QUANT_V2_NOT_ACTIVE');assert.equal(radar.marketRegime.state,'UNAVAILABLE');assert.equal((await materializedApi.getRadarIntelligence({lookback:7})).reason,'INVALID_RADAR_SELECTION');});
