...
test('Home composes the full product universe without live claims',async()=>{const before=reads.length,result=await api.getHomeIntelligence(['NVDA','TSLA']);assert.equal(result.market.observations.length,6875);assert.equal(result.watchlist.members.length,2);assert.equal(result.watchlist.partial,false);assert.equal(result.isLive,false);assert.equal(result.dataMode,'LATEST_AVAILABLE_EOD');assert.ok(reads.slice(before).includes('/quant/config/market-calendar.json'));});
test('Home calendar failure does not hide independent research data',async()=>{const offlineCalendar=Service.create({loadJSON:async p=>{if(p.endsWith('market-calendar.json'))throw Error('offline');return JSON.parse(await readFile(new URL(p.slice(1),root),'utf8'));},displayPolicy:Policy,queryEngine:Query});const result=await offlineCalendar.getHomeIntelligence([]);assert.equal(result.session.state,'UNAVAILABLE');assert.equal(result.market.observations.length,6875);});
