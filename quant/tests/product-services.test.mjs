...
test('optional breadth projection cannot take down the five connected intelligence rows',async()=>{
 for(const mode of ['missing','malformed']){const guarded=Service.create({loadJSON:async p=>{if(p==='/quant/data/product/capabilities-v1.json'){if(mode==='missing')throw Error('offline');return {version:'broken'};}return JSON.parse(await readFile(new URL(p.slice(1),root),'utf8'));},displayPolicy:Policy,queryEngine:Query});const u=await guarded.getUniverse();assert.equal(u.state,'AVAILABLE');assert.equal(u.stocks.length,5);assert.equal(u.scope,'FEATURED_FULL_INTELLIGENCE_SET');assert.equal(u.productCapabilityState,'UNAVAILABLE');assert.equal(u.productCapabilityReason,'CAPABILITY_SOURCE_UNAVAILABLE');assert.equal(u.productUniverseSize,null);assert.equal(u.capabilityCounts,null);}
});
