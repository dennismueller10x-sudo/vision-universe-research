...
const require=createRequire(import.meta.url),Service=require('../api/product-services.js'),Policy=require('../engines/display-policy.js'),Query=require('../engines/query.js'),Rules=require('../engines/rule-contract.js');
...
test('out-of-scope ticker gets measured capabilities without triggering raw or five-panel reads',async()=>{const before=reads.length;const s=await api.getStockIntelligence('TSLA');assert.equal(s.reason,'PARTIAL_PRODUCT_DATA');assert.equal(s.identityState,'AVAILABLE');assert.equal(s.availability.history.state,'AVAILABLE');assert.ok('liveSnapshot' in s.availability);assert.equal('realtime' in s.availability,false);assert.equal((await api.getRealtimeCapability('TSLA')).state,'AVAILABLE');assert.ok(reads.slice(before).every(p=>p.startsWith('/quant/data/universe/')||p==='/quant/data/product/capabilities-v1.json'||p==='/quant/config/development-preview.json'||p==='/quant/config/feature-gates.json'));});
test('optional breadth projection cannot take down the five connected intelligence rows',async()=>{
 for(const mode of ['missing','malformed']){const guarded=Service.create({loadJSON:async p=>{if(p==='/quant/data/product/capabilities-summary-v1.json'){if(mode==='missing')throw Error('offline');return {version:'broken'};}return JSON.parse(await readFile(new URL(p.slice(1),root),'utf8'));},displayPolicy:Policy,queryEngine:Query});const u=await guarded.getUniverse();assert.equal(u.state,'AVAILABLE');assert.equal(u.stocks.length,5);assert.equal(u.scope,'FEATURED_FULL_INTELLIGENCE_SET');assert.equal(u.productCapabilityState,'UNAVAILABLE');assert.equal(u.productCapabilityReason,'CAPABILITY_SOURCE_UNAVAILABLE');assert.equal(u.productUniverseSize,null);assert.equal(u.capabilityCounts,null);}
});
test('universe breadth loads only the compact summary; security capabilities remain lazy',async()=>{const isolatedReads=[];const service=Service.create({loadJSON:async p=>{isolatedReads.push(p);return JSON.parse(await readFile(new URL(p.slice(1),root),'utf8'));},displayPolicy:Policy,queryEngine:Query});const u=await service.getUniverse();assert.equal(u.productCapabilityState,'AVAILABLE');assert.ok(isolatedReads.includes('/quant/data/product/capabilities-summary-v1.json'));assert.ok(!isolatedReads.includes('/quant/data/product/capabilities-v1.json'));await service.getStockIntelligence('TSLA');assert.ok(isolatedReads.includes('/quant/data/product/capabilities-v1.json'));});
...
test('canonical Screener consumes the existing real Technical and Elliott snapshot index',async()=>{
 ...
 const expectedScored=expectedRows.filter(row=>row.opportunityScore>=65).sort((a,b)=>b.opportunityScore-a.opportunityScore||a.instrumentId.localeCompare(b.instrumentId));assert.ok(expectedScored.length>0);
 const scored=await api.screen(score);assert.equal(scored.state,'AVAILABLE');assert.deepEqual(scored.stocks.map(s=>s.ticker),expectedScored.map(row=>row.instrumentId));assert.equal(scored.predicateHash,Rules.predicateHash(Rules.fromQuery(score)));assert.ok(scored.stocks.every(stock=>{const source=expectedRows.find(row=>row.instrumentId===stock.ticker);return stock.technicalOpportunityScore.value===source.opportunityScore&&stock.technicalOpportunityScore.asOf===source.asOf;}));
 ...
 const expectedElliott=expectedRows.filter(row=>row.elliottStatus==='LOW_CONFIDENCE').sort((a,b)=>b.opportunityScore-a.opportunityScore||a.instrumentId.localeCompare(b.instrumentId));
 assert.deepEqual(elliott.stocks.map(s=>s.ticker),expectedElliott.map(row=>row.instrumentId));assert.ok(elliott.stocks.length>0);assert.ok(elliott.stocks.every(stock=>{const source=expectedRows.find(row=>row.instrumentId===stock.ticker);return stock.elliottCountStatus.value===source.elliottStatus&&stock.elliottCountStatus.asOf===source.asOf&&stock.elliottCountStatus.isProbability===false&&stock.elliottCountStatus.unit==='method_fit';}));
});
test('Technical snapshot projection preserves values and canonical ticker tie-breaks',async()=>{
 const guarded=Service.create({loadJSON:async p=>{const x=JSON.parse(await readFile(new URL(p.slice(1),root),'utf8'));if(p.endsWith('/technical/index.json')){const jpm=x.instruments.find(row=>row.instrumentId==='JPM'),msft=x.instruments.find(row=>row.instrumentId==='MSFT');Object.assign(jpm,{opportunityScore:91,elliottStatus:'LOW_CONFIDENCE',asOf:'2026-09-16'});Object.assign(msft,{opportunityScore:91,elliottStatus:'LOW_CONFIDENCE',asOf:'2026-09-17'});}return x;},displayPolicy:Policy,queryEngine:Query});
 const tied=await guarded.screen(Query.createQuery({filters:[{field:'technicalOpportunityScore',operator:'gte',value:91,scale:'raw'},{field:'elliottCountStatus',operator:'eq',value:'LOW_CONFIDENCE',scale:'raw'}],sort:[{field:'technicalOpportunityScore',direction:'desc'}]}));
 assert.equal(tied.state,'AVAILABLE');assert.deepEqual(tied.stocks.map(stock=>stock.ticker),['JPM','MSFT']);assert.deepEqual(tied.stocks.map(stock=>[stock.technicalOpportunityScore.value,stock.technicalOpportunityScore.asOf,stock.elliottCountStatus.value,stock.elliottCountStatus.asOf]),[[91,'2026-09-16','LOW_CONFIDENCE','2026-09-16'],[91,'2026-09-17','LOW_CONFIDENCE','2026-09-17']]);
});
...
test('Discover recipes reproduce identical editable Screener queries and preserve their defaults',async()=>{
 ...
 for(const c of discovered.collections){const screened=await api.screen(c.query);assert.equal(c.result.queryHash,screened.queryHash);assert.equal(c.predicateHash,screened.predicateHash);assert.deepEqual(c.predicate,screened.predicate);assert.deepEqual(c.result.stocks,screened.stocks);assert.equal(c.result.eligible,5);}
...
test('Home composes existing products without price history fanout or live claims',async()=>{const before=reads.length,result=await api.getHomeIntelligence(['NVDA','TSLA']);assert.equal(result.market.observations.length,5);assert.equal(result.watchlist.members.length,2);assert.equal(result.watchlist.partial,true);assert.equal(result.isLive,false);assert.equal(result.dataMode,'LATEST_AVAILABLE_EOD');assert.ok(reads.slice(before).every(p=>p==='/quant/config/market-calendar.json'));});
...
test('shared Screener and Atlas suppress raw quotes and prohibit price probing under derived-only grants',async()=>{
 ...
 const result=await tools.call('screenStocks',{query});assert.equal(result.ok,true);assert.equal(result.data.stocks.length,5);assert.ok(result.data.stocks.every(s=>s.price.value===null));
 ...
 const approved=await api.screen(Screener.build([{field:'price',operator:'gte',value:nvda.fundamentals.price,scale:'raw'}]));assert.equal(approved.state,'AVAILABLE');assert.ok(approved.stocks.some(s=>s.ticker==='NVDA'&&s.price.value===nvda.fundamentals.price));
});
...
test('Stock retains typed independent failures when both factor contract and chart are unavailable',async()=>{const partial=Service.create({loadJSON:async p=>{if(p.includes('/daily/'))throw Error('missing history');const x=JSON.parse(await readFile(new URL(p.slice(1),root),'utf8'));if(p.endsWith('quant-factor-inputs.json'))x.versions.buildScript='unknown';return x;},displayPolicy:Policy,queryEngine:Query});const result=await partial.getStockIntelligence('NVDA');assert.equal(result.state,'AVAILABLE');assert.equal(result.chart.state,'SOURCE_MISSING');assert.equal(result.quant.state,'UNAVAILABLE');});
