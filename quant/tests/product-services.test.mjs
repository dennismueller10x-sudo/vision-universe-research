...
test('Watchlist raw quote is suppressed under a derived-only grant',async()=>{const onlyDerived=Service.create({loadJSON:async p=>JSON.parse(await readFile(new URL(p.slice(1),root),'utf8')),displayPolicy:{...Policy,check:args=>({allowed:args.form==='derived'})},queryEngine:Query});const result=await onlyDerived.getWatchlistIntelligence(['NVDA']);assert.equal(result.members[0].price.value,null);assert.equal(result.members[0].price.reason,'DISPLAY_NOT_PERMITTED');assert.equal(result.members[0].momentum6m.value,23.734);});
...
test('shared Screener and Atlas suppress raw quotes and prohibit price probing under derived-only grants',async()=>{
 ...
 const approved=await api.screen(Screener.build([{field:'price',operator:'gte',value:200,scale:'raw'}]));assert.equal(approved.state,'AVAILABLE');assert.ok(approved.stocks.some(s=>s.ticker==='NVDA'&&s.price.value===225.73));
});
