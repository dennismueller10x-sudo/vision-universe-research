...
test('Strategy context uses canonical current breadth without enabling Quant V2 ranking or historical results',async()=>{const c=await api.getStrategyContext();assert.equal(c.state,'AVAILABLE');assert.equal(c.definition.execution.timing,'next_open');assert.equal(c.currentSelection.scope,'CANONICAL_PRODUCT_UNIVERSE');assert.equal(c.currentSelection.selectable,capability.members.length);assert.equal(c.currentSelection.ranking.state,'UNAVAILABLE');assert.equal(c.currentSelection.ranking.reason,'QUANT_V2_NOT_ACTIVE');/* Nicht gegen eine Zeichenkette, sondern gegen den veroeffentlichten
   Vertrag: der Dienst darf dem Nutzer keine andere Fassung nennen als die,
   nach der er rechnet. Die Fassung selbst ist in
   quant-v2-methodology.test.mjs festgenagelt. */
assert.equal(c.quantV2.methodologyVersion,quantV2Contract.methodologyVersion);
assert.equal(c.quantV2.methodologyVersion,require('../engines/factor-evidence.js').DERIVED_FROM);assert.equal(c.quantV2.status,'SPECIFIED_NOT_ACTIVE');assert.equal(c.quantV2.publicationAllowed,false);assert.deepEqual(c.quantV2.factorOrder,['quality','growth','momentum','value','profitability','revisions','risk']);assert.equal(c.quantV2.factorReadiness.revisions,'BLOCKED_EXTERNAL');assert.equal(c.backtest.state,'UNAVAILABLE');assert.equal(c.backtest.checks.length,5);assert.ok(!('results' in c.backtest));});
