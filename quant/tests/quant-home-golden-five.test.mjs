...
test("GF1 · Quant Home laedt das Golden-Five-Panel und verlinkt jeden Titel auf die Stock-Detail-Seite", () => {
  ...
  assert.match(app, /data\/sec\/quant-factor-inputs\.json/,
    "Quant Home muss dieselbe Datei lesen wie die Stock-Detail-Seite, nicht eine zweite Kopie");
  assert.match(app, /href:\s*S\.BASE\s*\+\s*"stock\/\?ticker="\s*\+\s*ticker/,
    "jede Golden-Five-Karte muss auf /quant/stock/?ticker=<TICKER> verlinken");
  assert.doesNotMatch(app, /\["AAPL"\s*,\s*"MSFT"/,
