...
const shardOf=p=>({schema:'vu-quant-quarterly-shard-1.0.0',shard:'05',issuers:{'0001318605':p}});
...
test('product service resolves canonical issuer before one release artifact; annual and TTM semantics stay separate',async()=>{
 ...
 const api=Service.create({loadJSON:async p=>{jsonReads.push(p);return load(p);},loadCompressedJSON:async p=>{compressedReads.push(p);return shardOf(projected);},displayPolicy:Policy,queryEngine:Query});
 const result=await api.getHistoricalFundamentals('TSLA',{period:'quarterly'});
 assert.equal(result.state,'AVAILABLE');assert.deepEqual(compressedReads,['/quant/data/sec/quarterly/05.json.gz']);assert.ok(!jsonReads.some(p=>p.includes('/discover/')));
...
test('production loader decompresses one same-origin artifact and rejects oversized expansion',async()=>{
 ...
 try{
  globalThis.fetch=async(url,options)=>{reads.push({url,options});return new Response(gzipSync(JSON.stringify(shardOf(projected))));};
  ...
  assert.equal(reads.length,1);assert.equal(reads[0].url,'/quant/data/sec/quarterly/05.json.gz');assert.equal(reads[0].options.credentials,'omit');
...
test('shard identity cannot substitute a neighbouring issuer',async()=>{
 for(const payload of [{...shardOf(projected),shard:'06'},{schema:'vu-quant-quarterly-shard-1.0.0',shard:'05',issuers:{'0000000005':projected}}]){
  const api=Service.create({loadJSON:load,loadCompressedJSON:async()=>payload,displayPolicy:Policy,queryEngine:Query});
  assert.equal((await api.getHistoricalFundamentals('TSLA',{period:'quarterly'})).state,'UNAVAILABLE');
 }
});
