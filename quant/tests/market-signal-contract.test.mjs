...
test('approved scope is preserved and denied raw display prevents history reads',async()=>{const result=await api.getSignals({lookback:60});assert.equal(result.results.length,5);assert.equal(result.partial,false);assert.equal(result.events.length,4);
 /* Die Zahl 4 gilt, weil oben auf CUTOFF festgepinnt wird - ohne den Pin
    waere sie eine Momentaufnahme des Datenstands und keine Eigenschaft des
    Vertrags, und genau das hat vier Marktdatenlaeufe gekostet. Zusaetzlich
    geprueft wird, was unabhaengig vom Stichtag zugesichert ist: dass jedes
    Ereignis aus dem zugelassenen Umfang stammt und seine Herkunft nennt. */
 ...
 let rawReads=0;const denied=Service.create({loadJSON:p=>{if(p.includes('/golden-preview/'))rawReads++;return Promise.resolve(readPinned(p));},displayPolicy:{...Policy,check:args=>({allowed:args.form==='derived'})},queryEngine:Query});assert.equal((await denied.getSignals()).state,'UNAVAILABLE');assert.equal(rawReads,0);});
