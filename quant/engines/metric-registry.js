...
const entries=deepFreeze([...Market.entries,...legacy,...technical]);
const api=Object.freeze({version:'1.2.0',entries,get:id=>entries.find(m=>m.metricId===id)||null,affectedBy:inputs=>entries.filter(m=>m.inputs.some(i=>inputs.includes(i))).map(m=>m.metricId)});
