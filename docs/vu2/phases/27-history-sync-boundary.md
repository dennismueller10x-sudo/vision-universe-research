...
- A remote dry-run must pass current operation-matched preflight before constructing the driver. Only a local filesystem dry-run remains unmetered. Operation, universe, provider and market must match; measured=true/offline=false and nonnegative safe-integer budgets are mandatory. Offline, hypothetical symbol-count and unmetered reports cannot authorize a real operation.

## Tests and gate
Eleven CLI boundary tests run in isolated temporary directories, with synthetic inputs only in tests and no external credentials/network. They cover successful push/restore/idempotency, cross-runner continuation, freshness, identity conflict at both ends and bar level, conflicting prices, invalid dates/duplicate identities, missing remote dry-run preflight and malformed inputs. Together with existing EOD gate and full-history compatibility:17 tests PASS. Existing broad Node and remote Quant/SEC gates required before acceptance. No visual changes or new screenshot claims.
