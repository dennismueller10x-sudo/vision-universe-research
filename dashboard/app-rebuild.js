(async function () {
  ...
  const rangeDays = [5, 22, 252, 756, 1260].includes(Number(params.get('days'))) ? Number(params.get('days')) : 756;
  ...
  try {
    ...
    if (view === 'charting') {
      ...
      if (market.status !== 'generated' || !Object.keys(market.symbols || {}).length || !market.public_data_state || market.public_data_state.display_allowed !== true) {
        app.innerHTML = `${provenance([['MARKET DATA', 'UNAVAILABLE'], ['FORM', 'PRECOMPUTED'], ['SOURCE', 'NONE']])}<div class="kicker">Charting</div><h1 class="heading">Daten nicht verfügbar</h1><p class="notice">Öffentlich speicherbare Kursdaten liegen derzeit nicht vor. Es wird nicht auf Mock-Kurse oder stille Nullwerte zurückgefallen. Für Charts ist ein geschützter interner Datenpfad erforderlich.</p>`;
        return;
      }
      ...
      let winStart = Math.max(0, totalLen - rangeDays), winEnd = totalLen - 1, curSide = side;
      const visible = { ema20: false, ema50: false, sma200: false, bollinger: false, zones: false, rsi: false, macd: false, stoch: false, volume: false, mcap: false };
      ...
      const buildChart = (s, e) => {
        ...
        const y = (v) => T + (domainHi - v) * (H - T - B) / (domainHi - domainLo);
        ...
        if (atPresent) {
          ...
          const zoneY = (v) => T + (domainHi - Math.min(Math.max(v, domainLo), domainHi)) * (H - T - B) / (domainHi - domainLo);
        ...
        const pctChange = closes[0] ? (closes.at(-1) - closes[0]) / closes[0] * 100 : null;
        const priceDir = (pctChange == null || pctChange >= 0) ? 'up' : 'down';
        return { grid, path, x, y, closes, e20, e50, s200, upper, lower, waves, wavePoints, zonesHtml, zonesInfo, rsiSvg, stochSvg, macdSvg, volSvg, mcapPath, mcapAxis, pctChange, priceDir, dateRange: dateLabel(s, e) };
      };
      ...
      document.getElementById('rangeNav').onclick = (e) => {
        ...
        winStart = Math.max(0, totalLen - days); winEnd = totalLen - 1;
