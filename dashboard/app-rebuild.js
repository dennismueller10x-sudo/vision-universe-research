(async function () {
  ...
  const rangeDays = [1, 22, 252, 756, 1260].includes(Number(params.get('days'))) ? Number(params.get('days')) : 756;
  ...
  try {
    ...
    if (view === 'charting') {
      ...
      const rangeNav = `<nav class="range"><a class="${rangeDays === 1 ? 'on' : ''}" href="?symbol=${stock.symbol}&days=1">1 Tag · Live</a><a class="${rangeDays === 22 ? 'on' : ''}" href="?symbol=${stock.symbol}&days=22">1 Monat</a><a class="${rangeDays === 252 ? 'on' : ''}" href="?symbol=${stock.symbol}&days=252">1 Jahr</a><a class="${rangeDays === 756 ? 'on' : ''}" href="?symbol=${stock.symbol}&days=756">3 Jahre</a><a class="${rangeDays === 1260 ? 'on' : ''}" href="?symbol=${stock.symbol}&days=1260">5 Jahre</a></nav>`;
      if (rangeDays === 1) {
        app.innerHTML = `<div class="kicker">Technische Analyse β</div><h1 class="heading">Charting</h1>${picker(stocks, stock.symbol, 'charting', 'chartSearch')}<div class="chart-head"><div><h2>${stock.symbol} · ${esc(stock.name)}</h2></div>${rangeNav}</div><div id="liveChartMount"></div>`;
        bindSearch('chartSearch');
        const mount = document.getElementById('liveChartMount');
        import('/dashboard/components/live-stock-chart.js')
          .then((mod) => mod.mountLiveStockChart(mount, { symbol: stock.symbol, name: stock.name }))
          .catch((error) => { console.error(error); mount.innerHTML = '<p class="notice">Live-Chart konnte nicht geladen werden.</p>'; });
        return;
      }
