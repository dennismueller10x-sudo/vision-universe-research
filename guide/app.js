(async function () {
  ...
  try {
    ...
    const metricRows = [['Marktkapitalisierung', fmtMoney(metric.market_capitalization)], ['Enterprise Value', fmtMoney(metric.enterprise_value)], ['KGV (TTM)', fmtRatio(metric.pe_ratio_ttm)], ['KGV (erwartet)', fmtRatio(metric.pe_ratio_forward)], ['KUV', fmtRatio(metric.ps_ratio_ttm)], ['EV/Sales', fmtRatio(metric.ev_to_revenue)], ['EV/EBITDA', fmtRatio(metric.ev_to_ebitda)], ['EV/FCF', fmtRatio(metric.ev_to_fcf)], ['KBV', fmtRatio(metric.price_to_book)], ['PEG', fmtRatio(metric.peg_ratio)], ['Bruttomarge', fmtPct(metric.gross_margin_pct)], ['Operative Marge', fmtPct(metric.operating_margin_pct)], ['Nettomarge', fmtPct(metric.net_margin_pct)], ['Eigenkapitalrendite', fmtPct(metric.return_on_equity_pct)], ['Umsatzwachstum YoY', fmtPct(metric.revenue_growth_yoy_pct)], ['Gewinnwachstum YoY', fmtPct(metric.earnings_growth_yoy_pct)], ['Verschuldungsgrad', fmtRatio(metric.debt_to_equity)], ['Dividendenrendite', fmtPct(metric.dividend_yield_pct)], ['Free Cashflow (TTM)', fmtMoney(metric.free_cash_flow_ttm)]];

    app.innerHTML = `<div class="kicker">Aktien Guide</div><h1 class="heading">Guide</h1>${picker}<div class="stock-head"><div><h2>${stock.symbol} · ${esc(stock.name)}</h2>${lastClose != null ? `<b>${fmt(lastClose, 2)} USD</b>` : ''}</div>${rangeNav}</div>${chartHtml}<h2 class="section-title">Fundamentale Kennzahlen</h2><section class="metrics">${metricRows.map(([name, value]) => `<div class="metric"><span>${name}</span><b>${value}</b></div>`).join('')}</section><p class="data-note">Datenstand ${metric.data_as_of ? esc(new Date(metric.data_as_of).toLocaleDateString('de-DE')) : 'noch nicht abgerufen'} · Quelle: Twelve Data (manuell angestoßen). Nicht verfügbare Werte werden nicht geschätzt.</p>`;
